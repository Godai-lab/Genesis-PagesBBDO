<x-app-layout>
    <x-slot name="title">Génesis - Modelos IA - Editar</x-slot>
    <x-slot name="header">
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="max-w-2xl mx-auto">
                        <form action="{{ route('ai-models.update',$aiModel->id)}}" method="POST" data-validate="true">
                            @csrf 
                            @method('PUT') 
                            ...
                            @php
                                ...
                                // Obtener valores del pricing actual si existe
                                $currentPricingType = old('pricing_type', $currentPricing ? $currentPricing->pricing_type : '');
                                $currentEffectiveFrom = old('effective_from', $currentPricing && $currentPricing->effective_from ? $currentPricing->effective_from->format('Y-m-d') : date('Y-m-d'));
                                $currentEffectiveTo = old('effective_to', $currentPricing && $currentPricing->effective_to ? $currentPricing->effective_to->format('Y-m-d') : '');
                                $currentMarkup = old('markup_percentage', $currentPricing ? $currentPricing->markup_percentage : '');
                                $currentPricingStatus = old('pricing_status', $currentPricing ? ($currentPricing->status === 'active') : true);
                                
                                // Obtener valores de unit_definition
                                $unitDef = $currentPricing ? ($currentPricing->unit_definition ?? []) : [];
                                $currentInputPrice = old('input_price', $unitDef['input_price'] ?? '');
                                $currentOutputPrice = old('output_price', $unitDef['output_price'] ?? '');
                                $currentPricePerGeneration = old('price_per_generation', $unitDef['price_per_generation'] ?? '');
                                $currentPricePerSecond = old('price_per_second', $unitDef['price_per_second'] ?? '');
                                $currentMinimumSeconds = old('minimum_seconds', $unitDef['minimum_seconds'] ?? 1);
                                $currentPricePerCredit = old('price_per_credit', $unitDef['price_per_credit'] ?? '');
                            @endphp

                            <div x-data="{ pricingType: '{{ $currentPricingType }}' }">
                                <x-dynamic-form 
                                    :fields="[
                                        ['label'=>'Proveedor','type'=>'select', 'name'=>'provider_id', 'id'=>'provider_id', 'col'=>'sm:col-span-4', 'value'=>old('provider_id', $aiModel->provider_id), 'attr'=>'data-validation-rules=required data-field-name=proveedor', 'list'=>$providerList],
                                        ['label'=>'Nombre','type'=>'text', 'name'=>'name', 'id'=>'name', 'col'=>'sm:col-span-4', 'value'=>old('name', $aiModel->name), 'attr'=>'data-validation-rules=required|max:255 data-field-name=nombre'],
                                        ['label'=>'Slug (Nombre real del modelo en API)','type'=>'text', 'name'=>'slug', 'id'=>'slug', 'col'=>'sm:col-span-4', 'value'=>old('slug', $aiModel->slug), 'attr'=>'data-field-name=slug', 'placeholder'=>'Ej: gemini3.0-flash'],
                                        ['label'=>'Tipo de modelo','type'=>'select', 'name'=>'model_type', 'id'=>'model_type', 'col'=>'sm:col-span-4', 'value'=>old('model_type', $aiModel->model_type), 'attr'=>'data-validation-rules=required data-field-name=tipo_de_modelo', 'list'=>$modelTypeList],
                                        ['label'=>'Disponible hasta','type'=>'date', 'name'=>'available_until', 'id'=>'available_until', 'col'=>'sm:col-span-4', 'value'=>old('available_until', $aiModel->available_until ? $aiModel->available_until->format('Y-m-d') : ''), 'attr'=>'data-field-name=disponible_hasta', 'placeholder'=>'Dejar vacío si es permanente'],
                                        ['label'=>'Estado','type'=>'switch', 'name'=>'status', 'id'=>'status', 'col'=>'sm:col-span-4', 'value'=>old('status', $aiModel->status === 'active')],
                                        ]" 
                                    >
                                    <h2 class="text-base font-semibold leading-7 text-black">Actualización de modelo IA</h2>
                                    <p class="mt-1 text-sm leading-6 text-black">Por favor, complete los siguientes campos:</p>
                                </x-dynamic-form>
                                
                                <x-dynamic-form 
                                    :fields="[
                                        ['label'=>'Fecha efectiva desde','type'=>'date', 'name'=>'effective_from', 'id'=>'effective_from', 'col'=>'sm:col-span-3', 'value'=>$currentEffectiveFrom, 'attr'=>'data-validation-rules=required data-field-name=fecha_efectiva_desde'],
                                        ['label'=>'Fecha efectiva hasta','type'=>'date', 'name'=>'effective_to', 'id'=>'effective_to', 'col'=>'sm:col-span-3', 'value'=>$currentEffectiveTo, 'attr'=>'data-field-name=fecha_efectiva_hasta'],
                                        ['label'=>'Margen de ganancia (%)','type'=>'number', 'name'=>'markup_percentage', 'id'=>'markup_percentage', 'col'=>'sm:col-span-3', 'value'=>$currentMarkup, 'attr'=>'step=0.01 min=0 max=100 data-field-name=margen_de_ganancia', 'placeholder'=>'Opcional'],
                                        ['label'=>'Estado del precio','type'=>'switch', 'name'=>'pricing_status', 'id'=>'pricing_status', 'col'=>'sm:col-span-3', 'value'=>$currentPricingStatus]
                                        ]" 
                                    >
                                            ...
                                            <div class="mt-2">
                                                ...
                                                >
                                                    ...
                                                    @foreach($pricingTypeList as $type)
                                                        <option value="{{ $type['id'] }}" {{ $currentPricingType == $type['id'] ? 'selected' : '' }}>
                                                            {{ $type['name'] }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                                        ...
                                                        <div class="sm:col-span-1">
                                                            ...
                                                            <div class="mt-2">
                                                                <input 
                                                                    ...
                                                                    step="0.01"
                                                                    min="0"
                                                                    value="{{ old('citation_price', $currentPricing->unit_definition['citation_price'] ?? '') }}"
                                                                    placeholder="2.00"
                                                                    class="block w-full rounded-lg border-1 py-1.5 text-black dark:text-gray-100 shadow-sm ring-1 ring-inset ring-gray-600 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-black dark:focus:ring-indigo-600 sm:text-sm sm:leading-6 bg-transparent dark:bg-gray-700"
                                                                    :disabled="pricingType !== 'per_token'"
                                                        ...
                                                        <div class="sm:col-span-1">
                                                            ...
                                                            <div class="mt-2">
                                                                <input 
                                                                    ...
                                                                    step="0.01"
                                                                    min="0"
                                                                    value="{{ old('reasoning_price', $currentPricing->unit_definition['reasoning_price'] ?? '') }}"
                                                                    placeholder="3.00"
                                                                    class="block w-full rounded-lg border-1 py-1.5 text-black dark:text-gray-100 shadow-sm ring-1 ring-inset ring-gray-600 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-black dark:focus:ring-indigo-600 sm:text-sm sm:leading-6 bg-transparent dark:bg-gray-700"
                                                                    :disabled="pricingType !== 'per_token'"
                                                        ...
                                                        <div class="sm:col-span-1">
                                                            ...
                                                            <div class="mt-2">
                                                                <input 
                                                                    ...
                                                                    step="0.01"
                                                                    min="0"
                                                                    value="{{ old('search_query_price', $currentPricing->unit_definition['search_query_price'] ?? '') }}"
                                                                    placeholder="5.00"
                                                                    class="block w-full rounded-lg border-1 py-1.5 text-black dark:text-gray-100 shadow-sm ring-1 ring-inset ring-gray-600 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-black dark:focus:ring-indigo-600 sm:text-sm sm:leading-6 bg-transparent dark:bg-gray-700"
                                                                    :disabled="pricingType !== 'per_token'"
                                        ...
                                        <div x-show="pricingType === 'per_generation'" x-cloak class="sm:col-span-4">
                                            ...
                                            <div class="mt-2">
                                                <input 
                                                    ...
                                                    step="0.01"
                                                    min="0"
                                                    value="{{ $currentPricePerGeneration }}"
                                                    placeholder="0.04"
                                                    class="block w-full rounded-lg border-1 py-1.5 text-black dark:text-gray-100 shadow-sm ring-1 ring-inset ring-gray-600 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-black dark:focus:ring-indigo-600 sm:text-sm sm:leading-6 bg-transparent dark:bg-gray-700"
