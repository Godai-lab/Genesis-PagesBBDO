...
class ImageEditor extends Component
{
    ...
    /**
     * Muestra el selector de cantidad de salidas en el editor.
     * Solo GPT usa `count` en la API; Gemini (Nano Banana) y Flux no.
     */
    public function getShowOutputCountSelectorProperty(): bool
    {
        return $this->model === 'gpt-image-1';
    }
    ...
    public function updateModel($key)
    {
        ...
        if (!$this->showOutputCountSelector && $this->count > 1) {
            $this->count = 1;
        }
    ...
    {
        try {
            $fallbackPollUrl = 'https://api.bfl.ai/v1/get_result?id=' . rawurlencode((string) $generationId);
            ...
            Log::info('🔍 Verificando estado de edición Flux desde frontend', [
                ...
                'fallback_get_result_url' => $fallbackPollUrl,
            ]);
            
            $result = \App\Services\FluxService::GetResult($generationId, $pollingUrl);
            
            Log::info('📡 Respuesta de FluxService::GetResult (edición)', [
                'generationId' => $generationId,
                'status' => $result['status'] ?? 'unknown',
                'hasData' => isset($result['data']),
                'flux_poll_raw_status' => $result['raw']['status'] ?? null,
            ]);
